...
void WINAPI Handler_SockChild(PTP_CALLBACK_INSTANCE, PVOID, PVOID, ULONG, ULONG_PTR, PTP_IO);
...
bool ManageSockets(UINT uCmd, PIOCP_ENV pEnv, PSOCK_ITEM psi, PTP_CALLBACK_INSTANCE pInst = NULL)
{
	...
	if (uCmd == CMD_REUSE)
	{
		...
		{
			cout << "DisconnectExfailed: " << WSAGetLastError() << endl;
			bSuccess = false;
			goto $LABEL_CLOSE;
		}
	}
	...
	if (uCmd == CMD_NEW)
	{
		psi->_ptpIo = CreateThreadpoolIo((HANDLE)psi->_sock, Handler_SockChild, pEnv, NULL);
		if (psi->_ptpIo == NULL)
			goto $LABEL_CLOSE;
	}

	StartThreadpoolIo(pEnv->_ptpIoLsn);
	pfnAcceptEx = (LPFN_ACCEPTEX)GetSockExtAPI(pEnv->_soListen, WSAID_ACCEPTEX);
	bIsOK = pfnAcceptEx
	(
		pEnv->_soListen, psi->_sock, psi->_buff, 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, NULL, (LPOVERLAPPED)psi
	);
...
void WINAPI Handler_SockListen(PTP_CALLBACK_INSTANCE pInst, PVOID pCtx, PVOID pov, ULONG dwErrCode, ULONG_PTR dwTrBytes, PTP_IO ptpLsnIo)
{
	PIOCP_ENV pie = (PIOCP_ENV)pCtx;
	PSOCK_ITEM psi = (PSOCK_ITEM)pov;
...
void _tmain()
{
	...
	try
	{
		...
		ie._ptpIoLsn = CreateThreadpoolIo((HANDLE)ie._soListen, Handler_SockListen, &ie, NULL);
