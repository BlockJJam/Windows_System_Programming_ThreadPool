...
VOID WINAPI RegMonCallback(PVOID pParam, BOOLEAN bTW)
{
	...
	(
		hKey, true, REG_MON_FLTR | REG_NOTIFY_THREAD_AGNOSTIC, hevNoti, true
	);
...
void _tmain()
{
	...
	try
	{
		...
		if (
			!RegisterWaitForSingleObject(&hRWSO, hevNoti, RegMonCallback, arMons, INFINITE, WT_EXECUTEDEFAULT)
			)
			throw (LONG)GetLastError();
